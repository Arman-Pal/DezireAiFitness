...
const ProfilePage = () => {
  // Get user and authentication status from Clerk
  const { user, isLoaded, isSignedIn } = useUser();

  // State for selected plan
  const [selectedPlanId, setSelectedPlanId] = useState<null | string>(null);

  // Always call useQuery, but skip execution if not authenticated
  const allPlans = useQuery(api.plans.getUserPlans, {}, { skip: !isLoaded || !isSignedIn });

  // Find the active plan
  const activePlan = allPlans?.find((plan) => plan.isActive);

  // Use selected plan if set, otherwise fall back to active plan
  const currentPlan = selectedPlanId
    ? allPlans?.find((plan) => plan._id === selectedPlanId)
    : activePlan;
  ...
  if (!isSignedIn || !user) {
    return <div>Please sign in to view your fitness plans</div>;
  }
